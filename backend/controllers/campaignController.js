...
/** Resolves a campaign's recipient list → [{ user_id, phone, firstName }]. */
const resolveAudience = async (def) => {
  ...
  if (def.audience === "all-women") {
    return females.map((f) => ({ user_id: f._id, phone: f.phone, firstName: firstNameOf(f.name) }));
  }
  ...
  return females
    ...
    .map((f) => ({ user_id: f._id, phone: f.phone, firstName: firstNameOf(f.name) }));
};
 ...
 * body: { mode: "test"|"send", testNumber?, batchSize? }
...
const send = async (req, res) => {
  try {
    ...
    const pending = audience.filter((r) => !sentPhones.has(r.phone)).slice(0, batchSize);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        remaining: Math.max(0, audience.length - totalSent),
      },
      statusCode: 200,
    });
  } catch (e) {
...
module.exports = { preview, send, CAMPAIGNS };
